#!/usr/bin/env python3
"""
Hand-merge artifact for PR #71 (multi-llm-integration-plan, Grandmasrecipes).
...
WHAT THIS APPLIES (Grandmasrecipes):
  ...
  Config (with .bak): .claude/settings.json, .claude/skill-rules.json,
                       ...
                       .gitignore (PR appends entries)

NOT TOUCHED — handle manually:
  - CLAUDE.md: my v2.0 trim restructured it into a lean hub. Add references
    to the new skills under "Essential Reading" — script prints a suggested
    block at the end.

REQUIREMENTS:
  - Run from inside the Grandmasrecipes repo on current main
  - PR #71 branch fetched: `git fetch origin claude/multi-llm-integration-plan-MZxEu`
...
HOW TO APPLY:
  ...
  git commit -m "Merge PR #71: multi-llm + recipe skills (15 new)"
  # Then close PR #71 with comment linking to this commit.
"""
...
PR_HEAD_SHA = '80e28cf79cad7c6e50c2f126da66d90f7beebad8'
...
def main():
    ...
    print('  git commit -m "Merge PR #71: multi-llm + recipe skills (15 new)"')
    print('  # Close PR #71 with link to this commit.')
